...
const FloatingFormula: React.FC<{ item: any, pool: string[] }> = ({ item, pool }) => {
  ...
  return (
     <div
        ...
        style={{
          left: `${item.left}%`, top: `${item.top}%`, fontSize: `${1.0 + (item.scale * 0.4)}rem`, 
          filter: `blur(${item.blur}px)`, opacity: item.opacity,
          animation: `comet-move ${item.duration}s linear infinite`, animationDelay: `${item.delay}s`,
...
const MarqueeImage = React.memo(({ src, className }: { src: string, className?: string }) => {
  ...
  return (
    <div className={`relative overflow-hidden bg-black/40 border border-white/5 rounded-2xl ${className}`}>
        <img 
            src={src} alt="" loading="lazy" decoding="async" onLoad={() => setLoaded(true)}
    ...
    for (let i = 0; i < 14; i++) {
        ...
        while (!placed && attempts < 2000) {
            ...
            if (!overlap) {
                ...
                let blur = 0; if (scale > 1.2) blur = (scale - 1.2) * 1.5;
                if (l < 45 && t > 20 && t < 75) blur += 1.5;
                pos.push({
                    left: l, top: t, duration: 6 + (Math.random() * 2), delay: -Math.random() * 10,
                    scale, blur, opacity: scale > 1.2 ? 0.35 : 0.5, formula: shuffledFormulas[i % shuffledFormulas.length]
                });
            ...
            <>
               <style>{`
                 @keyframes comet-move {
                    ...
                    100% { opacity: 0; transform: translate(120px, 120px) scale(var(--scale)); }
                 }
               `}</style>
