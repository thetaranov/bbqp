...
function App() {
  ...
  // --- НАЧАЛО ИЗМЕНЕНИЙ: Выносим JSX с хуками в useMemo ---
    ...
    for (let i = 0; i < 16; i++) {
        ...
        while (!placed && attempts < 2000) {
            ...
            if (!overlap && !imageOverlap) {
                ...
                pos.push({ left: l, top: t, duration: 6 + (Math.random() * 8), delay: -Math.random() * 10, scale, blur, opacity: scale > 1.2 ? 0.35 : 0.5, formula: PHYSICS_FORMULAS[i % PHYSICS_FORMULAS.length] });
  ...
  const ConfiguratorPanel = () => ( /* ... код ConfiguratorPanel ... */ );
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-black text-white selection:bg-orange-500 selection:text-white relative">
      ...
      <main className={`snap-container h-full w-full transition-opacity duration-1000 opacity-100`}>
        ...
        {/* --- НАЧАЛО ИЗМЕНЕНИЙ: Рендерим переменные вместо JSX --- */}
