...
interface ChefBotProps {
  ...
  showFloatingButton?: boolean; // Новый проп для скрытия кнопки
}
...
const ChefBot: React.FC<ChefBotProps> = ({ 
  ...
  externalIsOpen, 
  onToggle,
  showFloatingButton = true // По умолчанию true, но мы будем передавать false
}) => {
  const [internalIsOpen, setInternalIsOpen] = useState(false);
  const [input, setInput] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Здравствуйте! Я ИИ-консультант bbqp. Могу подсказать рецепт, ответить на вопросы о мангале или помочь с выбором.' }
  ]);
  ...
  // Если бот невидим вообще (visible=false)
  if (!visible && !isOpen) return null;

  return (
    <div 
      className={`fixed bottom-6 right-6 z-[90] flex flex-col items-end transition-all duration-1000 ${visible || isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
    >
      {isOpen && (
      ...
      {/* Рендерим кнопку только если showFloatingButton = true */}
      {showFloatingButton && (
        <button
