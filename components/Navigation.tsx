...
const Navigation: React.FC<NavigationProps> = ({ activeSection, isIntroComplete = true, onChatToggle }) => {
  ...
  return (
    <nav ref={navRef} className={`fixed top-0 left-0 right-0 z-[9999] transition-all duration-1000 py-4 shadow-sm ${bgClass} ${blurClass} ${isIntroComplete ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
      <div className="max-w-[95rem] mx-auto px-6 lg:px-8 flex justify-between items-center min-h-[40px]">
        ...
        <div className="hidden lg:flex items-center relative rounded-full p-1 border transition-colors duration-500 bg-white/5 border-white/5 bg-gradient-to-b from-white/5 to-transparent">
          <div className="absolute h-[calc(100%-8px)] top-1 bg-orange-600 rounded-full shadow-[0_0_20px_rgba(234,88,12,0.5)]" style={{ left: indicatorStyle.left, width: indicatorStyle.width, opacity: indicatorStyle.opacity, transitionProperty: 'left, width, opacity', transitionDuration: '350ms', transitionTimingFunction: 'cubic-bezier(0.2, 0, 0.2, 1.4)' }} />
          {NAV_LINKS.map((link, index) => (
            <a key={link.name} href={link.href} ref={(el) => { itemsRef.current[index] = el; }} onClick={(e) => handleScrollTo(e, link.href)} className={`relative z-10 px-5 py-2 text-sm font-medium transition-all duration-500 ${activeSection === link.href.substring(1) ? 'text-white drop-shadow-md' : inactiveColorClass}`}>
              {link.name}
            </a>
          ))}
        </div>
        ...
        <div className="hidden lg:block">
           <a href="https://t.me/thetaranov" target="_blank" rel="noopener noreferrer" className="relative overflow-hidden px-6 py-2.5 rounded-full text-sm font-bold transition-all active:scale-95 bg-white text-black hover:bg-gray-200 shadow-[0_0_20px_rgba(255,255,255,0.2)]">
            <span className="relative z-10">Собрать свой</span>
          </a>
        </div>

        <div className="lg:hidden absolute right-6 flex items-center gap-4">
           <button className={`p-1 transition-colors ${logoColorClass} hover:text-orange-500`} onClick={onChatToggle} aria-label="Support"><MessageSquare size={26} strokeWidth={2} /></button>
           <button className={`p-1 transition-colors ${logoColorClass}`} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label="Menu">{isMobileMenuOpen ? <X size={28} /> : <Menu size={28} />}</button>
        </div>
      </div>

      {isMobileMenuOpen && (
        <div className={`lg:hidden absolute top-full left-0 w-full border-t p-6 flex flex-col gap-2 shadow-2xl animate-fade-in h-screen z-40 ${mobileMenuBg} bg-gradient-to-b from-white/5 to-transparent backdrop-blur-2xl border-white/10 overflow-y-auto`}>
          {NAV_LINKS.map((link) => (
