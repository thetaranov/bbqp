...
const Navigation: React.FC<NavigationProps> = ({ activeSection, isIntroComplete = true, onChatToggle, onMenuToggle }) => {
      ...
      <div className="max-w-[95rem] mx-auto px-6 lg:px-8 flex justify-between items-center min-h-[40px]">
        ...
        <div className="hidden lg:flex items-center relative rounded-full p-1 border transition-colors duration-500 bg-white/5 border-white/5 bg-gradient-to-b from-white/5 to-transparent">
          ...
          {NAV_LINKS.map((link, index) => (
            <a key={link.name} href={link.href} ref={(el) => { itemsRef.current[index] = el; }} onClick={(e) => handleScrollTo(e, link.href)} className={`relative z-10 px-5 py-2 text-sm font-medium transition-all duration-500 ${activeSection === link.href.substring(1) ? 'text-white drop-shadow-md' : inactiveColorClass}`}>{link.name}</a>
          ))}
        </div>

        <div className="hidden lg:block"><a href="https://t.me/thetaranov" target="_blank" rel="noopener noreferrer" className="relative overflow-hidden px-6 py-2.5 rounded-full text-sm font-bold transition-all active:scale-95 bg-white text-black hover:bg-gray-200 shadow-[0_0_20px_rgba(255,255,255,0.2)]"><span className="relative z-10">Собрать свой</span></a></div>
